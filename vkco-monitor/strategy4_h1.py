...
def evaluate(c:list[Any])->dict[str,Any]:
    ...
    struct=structural_levels(c); return {"strategy":"S4_ADAPTIVE_H1","timeframe":"H1","mode":"SHADOW","candle":z.end.isoformat(),"price":z.close,"support":round(lv["support"],2),"resistance":round(lv["resistance"],2),"local_levels":{"support":round(lv["support"],2),"resistance":round(lv["resistance"],2),"lookback_h1":20},"structural_levels":struct,"avg_range":round(lv["avg_range"],4),"decision":"READY" if signal else "WAIT","reason":"TRIGGER_CONFIRMED" if signal else "NO_TRIGGER","signal":signal}
