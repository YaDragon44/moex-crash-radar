...
UA='Mozilla/5.0 OFZ-Signal-Radar/0.5.3'
...
def num(s):
    ...
    m=re.search(r'-?\d+(?:\.\d+)?',str(s).replace('\xa0',' ').replace(' ','').replace(',','.'));return float(m.group()) if m else None
...
def moex_block(obj,name):
    b=obj.get(name) or {};cols=b.get('columns') or [];data=b.get('data') or [];return dict(zip(cols,data[0])) if data else {}
...
def fetch_bondization(secid):
    ...
    for x in moex_rows(o,'coupons'):
        ...
        raw=x.get('value') if x.get('value') is not None else x.get('VALUE') if x.get('VALUE') is not None else x.get('couponvalue') if x.get('couponvalue') is not None else x.get('COUPONVALUE');out[str(d)]={'date':str(d),'value':num(raw)}
    return [out[k] for k in sorted(out)]
def fetch_bond(label,secid):
    o=json.loads(get(f'https://iss.moex.com/iss/engines/stock/markets/bonds/boards/TQOB/securities/{secid}.json?iss.meta=off&iss.only=securities,marketdata'));s=moex_block(o,'securities');m=moex_block(o,'marketdata');schedule=fetch_bondization(secid)
    price=m.get('LAST') or m.get('MARKETPRICE') or m.get('LCURRENTPRICE') or s.get('PREVPRICE');ytm=m.get('YIELD') or m.get('EFFECTIVEYIELD')
...
def main():
    ...
    strategy_path=[round(float(y10),2),15.0,13.0,11.0,9.0,5.0] if y10 is not None else []
    strategy=compare_strategies(portfolio,bonds,valuation_date,strategy_path) if portfolio.get('ready') and pq.get('ok') and strategy_path else {'ready':False}
    system_ok=pq.get('ok') and sanity and strategy.get('ready')
    snap={'version':'0.5.3','fetchedAt':now.isoformat(),'valuationDate':valuation_date,'curve':curve,'curveHistory':[],'keyRate':kr,'inflation':inf,'inflationExpectations':None,'minfinAuction':None,'budget':None,'cbTone':None,'bonds':bonds,'portfolio':portfolio,'portfolioQuality':{**pq,'economicSanity':sanity},'portfolioScenarios':scenarios,'strategyComparison':strategy,'rawRegime':raw,'confirmedRegime':confirmed,'transition':{'text':'LIVE SNAPSHOT'},'macro':{'known':known},'quality':{'market':'LIVE' if all(b['quality'] in ('LIVE','DELAYED') for b in bonds) else 'STALE','curve':'LIVE','macro':'PARTIAL','system':'LIVE' if system_ok else 'DEGRADED'},'sources':{'inflationExpectations':'N/A','minfinAuctions':'N/A','budget':'N/A','cbTone':'N/A','curve':'CBR','bonds':'MOEX','cashflows':'MOEX Bondization + safe synthetic extension','strategy':'same cashflow engine; protective sleeve 0%'}}
    OUT.parent.mkdir(parents=True,exist_ok=True);OUT.write_text(json.dumps(snap,ensure_ascii=False,indent=2),encoding='utf-8')
    print(f"OFZ SNAPSHOT PASS version=0.5.3 date={curve['date']} y10={y10} regime={raw} scenarios={len(scenarios)} strategy={strategy.get('ready')} system={system_ok}")
