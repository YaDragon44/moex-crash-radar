...
class EntryRadarInput:
    ...
    oi_regime: str  # DELEVERAGING | STABLE | MODERATE_BUILD | OVERHEATED | N/A
...
def evaluate(x: EntryRadarInput) -> EntryRadarOutput:
    if x.position_open:
        return EntryRadarOutput("MANAGE", "MANAGE POSITION", 0.0,
    ...
    if x.stop_atr is None or x.stop_atr <= 0 or x.stop_atr > 2:
        return EntryRadarOutput("NO_TRADE", "WAIT", 0.0,
            ["Risk gate failed", "Stop distance is invalid or >2 ATR", "Entry is too late or structure unclear"], "Valid stop <=2 ATR")
    ...
    if x.new_local_low_4h:
        return EntryRadarOutput("WATCH" if x.fgi <= 35 else "NO_TRADE", "WAIT", 0.0,
            ["Fear zone is present" if x.fgi <= 35 else "Crowd gate is weak", "BTC still makes a local low", "Price reversal is not confirmed"], "BTC stops making local lows")

    if x.fgi <= 35 and not x.price_confirm_4h:
        return EntryRadarOutput("ARMED", "WAIT FOR 4H CONFIRMATION", 0.0,
            ["F&G permits long search", "BTC has stopped making a local low", "4H breakout confirmation is still absent"], "New local low")
    ...
    return EntryRadarOutput("NO_TRADE", "WAIT", 0.0,
        ["No validated MVP setup", "Crowd/price combination is insufficient", "Capital preservation has priority"], "Wait for next setup")


def to_dict(x: EntryRadarInput):
    return {"input": asdict(x), "output": asdict(evaluate(x))}
