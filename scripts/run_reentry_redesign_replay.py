...
def main() -> None:
    ...
    event_days = set(((payload.get('frozen_exit_validation') or {}).get('event_days')) or [])
    if not event_days:
        raise SystemExit('historical_evidence frozen_exit_validation.event_days is missing or empty')
