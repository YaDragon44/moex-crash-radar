...
def build_bull_bubble_transitions(bull: Sequence[HistoricalBullPoint], bubble: Sequence[HistoricalBubblePoint]) -> list[BullBubbleTransition]:
    ...
    for p in bull:
        ...
        if q is None or p.score is None or q.score is None:
            out.append(BullBubbleTransition(p.day,p.score,None if q is None else q.score,None,"DATA_INSUFFICIENT")); continue
        spread=round(p.score-q.score,2)
        if p.score>=70 and q.score<45: state="HEALTHY_STRONG_BULL"
        elif p.score>=70 and q.score>=60: state="BUBBLE_BUILD_UP"
        elif p.score<60 and q.score>=75: state="DISTRIBUTION_WATCH"
        elif p.score<40 and q.score>=60: state="BREAKDOWN_RISK"
        elif q.score>=60 and p.transition in {"WEAKENING","WEAKENING_FAST"}: state="BULL_BUBBLE_DIVERGENCE"
        else: state="NEUTRAL"
