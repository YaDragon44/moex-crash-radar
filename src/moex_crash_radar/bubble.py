...
def calculate_bubble(inputs: BubbleInputs, history: Sequence[float] = ()) -> BubbleResult:
    ...
    # Bubble warnings need broad independent evidence. Do not infer a bubble
    # from valuation alone or from one fast market indicator.
    if groups < 4 or available_weight < 0.60:
        return BubbleResult(None, BubbleState.DATA_INSUFFICIENT, round(available_weight, 4), groups, None, 0, "N/A", ())
    ...
    return BubbleResult(score, state, round(available_weight, 4), groups, velocity, persistence, transition, reasons)
