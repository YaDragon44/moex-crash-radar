...
h=F.historicalPE({candles,epsSeries:[[2022,10],[2023,12],[2024,15.5]],priceSource:'MOEX ISS'});assert.equal(h.verified,false);assert.equal(h.points.length,0);assert(h.missing.includes('eps_2022_unverified'));
h=F.historicalPE({candles,epsSeries:[{year:2022,eps:10,verified:true},{year:2023,eps:12,verified:true,source:'Issuer',asOf:'2023-12-31'},{year:2024,eps:15.5,verified:true,source:'Issuer',asOf:'2024-12-31'}],priceSource:'MOEX ISS'});assert.equal(h.verified,false);assert(h.missing.includes('eps_2022_unverified'));
console.log('multiples data factory explicit EPS provenance tests: PASS');
