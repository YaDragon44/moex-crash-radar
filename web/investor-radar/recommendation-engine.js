// Investor Radar R1.8.0 — Recommendation Engine Foundation
// FACT != ANALYSIS != DECISION. Missing critical data => LOCK / НАБЛЮДАТЬ.
(function(global){
  'use strict';

  const ACTIONS = Object.freeze({
    BUY:'ПОКУПАТЬ', ADD:'ДОБИРАТЬ', HOLD:'ДЕРЖАТЬ', NO_ADD:'НЕ ДОБИРАТЬ',
    REDUCE:'СОКРАЩАТЬ', SELL:'ПРОДАВАТЬ', WATCH:'НАБЛЮДАТЬ', LOCK:'LOCK'
  });
  ...
  function decide(input){
    ...
    if(gate.hardStop){
      return {
        ticker, light:LIGHT.RED, action:ACTIONS.SELL, confidence:'СРЕДНЯЯ',
        valuation:'НЕ ОПРЕДЕЛЕНА',
        why:['Сработал критический риск-gate.'],
        facts:arr(input?.facts), risks:arr(input?.risk?.items),
        trigger:'Пересмотр после снятия/переоценки критического риска.',
        gate
      };
    }
    ...
    if(!gate.ok){
      return {
        ticker, light:LIGHT.GRAY, action:ACTIONS.WATCH, confidence:'НИЗКАЯ',
        valuation:'НЕ ОПРЕДЕЛЕНА',
        why:['Недостаточно данных для обоснованного вывода.','Не пройдены критические data gates: '+gate.missing.join(', ')+'.'],
        facts:arr(input?.facts), risks:arr(input?.risk?.items),
        trigger:'Появление недостающих подтверждённых данных.',
        gate
      };
    }
    ...
    const riskScore=finite(input?.risk?.score)?Number(input.risk.score):50;
    ...
    return {
      ticker, light, action, confidence:confidence(input,gate),
      valuation:zone==='ATTRACTIVE'?'ПРИВЛЕКАТЕЛЬНО':zone==='FAIR'?'СПРАВЕДЛИВО':zone==='EXPENSIVE'?'ДОРОГО':'НЕ ОПРЕДЕЛЕНА',
      why,
      facts:arr(input.facts), risks:arr(input?.risk?.items),
      trigger: input?.trigger || 'Изменение фундаментального тренда, оценки или ключевого риска.',
      gate,
      diagnostics:{zone,growthCagr:growth,hiddenValueGap:gap,riskScore}
    };
  }
