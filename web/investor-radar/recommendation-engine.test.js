...
x=base();delete x.portfolio;r=E.decide(x);assert.equal(r.action,'НАБЛЮДАТЬ');assert.equal(r.light,'ЖЁЛТЫЙ');assert.equal(r.confidence,'ВЫСОКАЯ');assert.equal(r.gate.ok,true);assert.equal(r.gate.personalActionOk,false);assert(r.gate.personalMissing.includes('portfolio_context'));assert.equal(r.diagnostics.portfolioKnown,false);assert.equal(Object.hasOwn(r.diagnostics,'held'),false);
